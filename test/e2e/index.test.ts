...
test(
  "app client reconnects websocket if closed before making a zome call",
  withApp(
    async (testCase) => {
      const { app_ws, cell_id } = testCase;
...
test(
  "app client fails to reconnect to websocket if closed before making a zome call if the provided token is invalid",
  withApp(async (testCase) => {
    const { app_ws, cell_id } = testCase;

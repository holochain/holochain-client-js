...
test(
  ...
  withConductor(ADMIN_PORT, async (t) => {
    ...
    const state = await admin.dumpState({
      cell_id,
    });
    t.equal(state[0].integration_dump.integrated, 7);
    t.equal(state[0].source_chain_dump.elements.length, 3);

    let fullState = await admin.dumpFullState({
      cell_id,
      dht_ops_cursor: undefined,
    });

    t.equal(fullState.integration_dump.integrated.length, 7);
    t.equal(fullState.integration_dump.dht_ops_cursor, 7);

    fullState = await admin.dumpFullState({
      cell_id,
      dht_ops_cursor: fullState.integration_dump.dht_ops_cursor,
    });
    t.equal(fullState.integration_dump.integrated.length, 0);
  })
);

...
test.skip(
  ...
  withConductor(ADMIN_PORT, async (t: Test) => {
    // withConductor(ADMIN_PORT_1, async (t: Test) => {
      const { installed_app_id, role_id, client, admin } = await installAppAndDna(
        ADMIN_PORT
      );
      const createCloneCellParams: CreateCloneCellRequest = {
...
test("how do cloned cells work?", async (t: Test) => {
  const conductor1 = await launch(ADMIN_PORT);
  const conductor2 = await launch(ADMIN_PORT_1);
  
  const c1 = await installAppAndDna(ADMIN_PORT)
  const c2 = await installAppAndDna(ADMIN_PORT_1)

  console.log('c1 cell_id', c1.cell_id)
  console.log('c2 cell_id', c2.cell_id)

  
  const createCloneCellParams: CreateCloneCellRequest = {
    app_id: c1.installed_app_id,
    role_id: c1.role_id,
    modifiers: {
      network_seed: "clone-0",
    },
  ...
  const cloneCell1 = await c1.client.createCloneCell(createCloneCellParams);
  
  console.log('c1.app_info', (await c1.client.appInfo({ installed_app_id: c1.installed_app_id })).cell_data.map(c => c.role_id))
  console.log('c2.app_info', (await c2.client.appInfo({ installed_app_id: c2.installed_app_id })).cell_data.map(c => c.role_id))

  await c2.client.createCloneCell(createCloneCellParams);

  console.log('cloning in c2')

  console.log('c1.app_info', (await c1.client.appInfo({ installed_app_id: c1.installed_app_id })).cell_data.map(c => c.role_id))
  console.log('c2.app_info', (await c2.client.appInfo({ installed_app_id: c2.installed_app_id })).cell_data.map(c => c.role_id))


  await c1.client.archiveCloneCell({
    app_id: c1.installed_app_id,
    clone_cell_id: cloneCell1.cell_id,
  });

  await c1.admin.deleteArchivedCloneCells({ app_id: c1.installed_app_id, role_id: c1.role_id });

  console.log('deleting in c1')

  console.log('c1.app_info', (await c1.client.appInfo({ installed_app_id: c1.installed_app_id })))
  console.log('c2.app_info', (await c2.client.appInfo({ installed_app_id: c2.installed_app_id })))
